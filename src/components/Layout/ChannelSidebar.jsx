...
function DMRow({ profile, active, onClick, onRemove }) {
  const [hov, setHov] = useState(false);
  if (!profile) return null;
  return (
    <div onMouseEnter={()=>setHov(true)} onMouseLeave={()=>setHov(false)}
      ...
      <div onClick={onClick} style={{ display:'flex', alignItems:'center', gap:10, flex:1, minWidth:0 }}>
        <div style={{ position:'relative', flexShrink:0 }}>
          ...
          <div style={{ position:'absolute', bottom:-1, right:-1, width:10, height:10, borderRadius:'50%', background: profile.status==='online'?'var(--online)':'var(--offline)', border:'2px solid var(--panel)' }} />
        </div>
        <div style={{ flex:1, minWidth:0 }}>
          <div style={{ fontSize:14, fontWeight: active?600:400, color: active?'var(--text)':'var(--text-muted)', whiteSpace:'nowrap', overflow:'hidden', textOverflow:'ellipsis' }}>{profile.displayName}</div>
        </div>
      </div>
      {/* Remove DM button — shows on hover */}
      {hov && (
        <button
